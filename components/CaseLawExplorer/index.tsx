...
import {getFetchSchema, getFilterSchema, LAYOUT_SCHEMA, NODE_SIZE_RANGE_MAP, VIEW_CONFIG_SCHEMA} from './constants'
...
const overrideTheme = theme => {
  Object.keys(theme.typography).forEach(key => {
    if (theme.typography[key]?.fontSize) {
      theme.typography[key].fontSize = Number(theme.typography[key].fontSize.replace('rem', '')) * 16 * 0.8
    }
  })
  return theme
}
          ...
          const call = async () => {
            ...
            if (elementData && !R.isEmpty(elementData)) {
              controller.update((draft, {graphEditorRef}) => {
                const {selectedItem} = getSelectedElementInfo(draft, graphEditorRef.current)
                if (selectedItem) {
                  selectedItem.data = {
                    ...selectedItem.data,
                    ...elementData
        ...
        case EVENT.SETTINGS_FORM_CHANGED: {
          ...
          if (payload.form.schema.title === FILTER_SCHEMA.schema.title) {
            ...
            draft.graphConfig.nodes.filter = {
              test: ({element, item}) => {
                const {year, degree, indegree, outdegree, isResult, community} = payload.value
                const {
                  networkStatistics: {local: localNetworkStatistics}
                } = graphEditor.context
                const stats = localNetworkStatistics?.[item.id] ?? {}
        ...
        <FullGraphContext.Provider value={{fullGraph, updateFullGraph}}>
          <ControllerContext.Provider value={{controller, activeCluster: controllerProps.activeCluster}}>
            <GraphEditor
              ...
              renderEdge={props => <RenderEdge {...props} graphEditorRef={controllerProps.ref} />}
              {...rest}
            />
